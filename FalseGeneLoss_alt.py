...
# used modules
import gffutils, os, sqlite3, re, argparse, sys, pickle, subprocess, copy
from Bio.SeqIO import FastaIO
from multiprocessing import Pool
from itertools import islice

# global commands
paftools_command_s = "k8 /disk3/juwan_d3/minimap2/misc/paftools.js splice2bed"  # version: included in minimap2 2.17-r974-dirty
minimap2_command_s = "/disk3/juwan_d3/minimap2/minimap2"  # version: 2.17-r974-dirty
bedtools_command_s = "/disk1/juwan_d1/bedtools2/bin/bedtools"  # version: v2.27.0
samtools_command_s = "samtools"  # version: 1.9-177-g796cf22
halLiftover_commnd_s = "halLiftover"  # version: halLiftover v2.1:
newcpgreport_command_s = "newcpgreport"  # version: EMBOSS:6.6.0.0
makeblastdb_command_s = "makeblastdb"  # version: 2.6.0+
blastn_command_s = "blastn"  # version: 2.6.0+
blat_command_s = "blat"  # version: v. 36x2

# global path
cat_rerun_dir_s = os.path.join("/", "disk1", "juwan_d1", "cat_rerun")
...
def tmp_db_maker(gff_s, name_s, working_dir_s, featuretype_s):
    """
    function to return pre-built annotation DB or newly generate it if not exist
    :param gff_s: string, path to gff file
    :param name_s: string, name of annotation DB
    ...
    # (1) if there's no pre-bulit annotation DB:
        ...
        dir_check(working_dir_s)
        # (1-1) try to generate new DB
        dialect = gffutils.DataIterator(gff_s).dialect
        dialect['fmt'] = 'gtf'
...
def scaffold_2_size_dictionary(mode_s):
    ...
    faidx_file_s = fasta_file_s + ".fai"
    if file_check(faidx_file_s) is False:
        os.system("%s faidx %s" % (samtools_command_s, fasta_file_s))
...
def gene_and_transcript_records(mode_s):
    """
    ...
    try:  # try to get pre_stored pickles
        ...
        if mode_s == "ref":
            ...
            for gene_rec in refDB.all_features(featuretype="gene"):
                gene_count_i += 1
                for mRNA_rec in refDB.children(id=gene_rec.id, featuretype="transcript"):
                    print(str(mRNA_rec))
                    trans_id = mRNA_rec.attributes["gene_id"][0]
                    gene_rec_2_transcript_id_d[str(gene_rec)] = trans_id
            ...
            print(gene_count_i)
            print(len(gene_rec_2_transcript_id_d))
            # (1-2) exclude duplicated transcript ID
            for gene_rec in refDB.all_features(featuretype="gene"):
                trans_id = gene_rec_2_transcript_id_d[str(gene_rec)]
                if trans_id not in duplicated_transcript_ids_l:
                    gene_recs_l.append(gene_rec)
            for gene_rec in gene_recs_l:
                transcript_recs_l.append([mRNA_rec for mRNA_rec in refDB.children(id=gene_rec.id, featuretype="transcript")][0])
        # (2) Old
        else:
            assert mode_s == "tgt"
...
def get_new_name(gene_rec, mode_s):
    ...
    if mode_s == "ref":
        ...
        for trans_rec in tgtDB.children(id=gene_rec.id, featuretype="transcript"):
            print(trans_rec.attributes["source_transcript"][0])
...
class FalseGeneLoss():
    ...
    def gff2cds(self, info_l):
        ...
        if mode_s == "ref":
            ...
            DB = tgtDB
        gene_id_s = gene_rec.id
        gene_name_s = gene_id_2_gene_name_d[mode_s][gene_id_s] #get_new_name(gene_rec=gene_rec, mode_s=mode_s)
        ...
        if eukaryote_bool is True:
            exon_recs_l = sorted([exon_rec for exon_rec in DB.children(id=gene_id_s, featuretype="exon")], key=lambda exon_rec: exon_rec.start)
        else:
            assert eukaryote_bool is False
            exon_recs_l = sorted([exon_rec for exon_rec in DB.children(id=gene_id_s, featuretype="CDS")], key=lambda exon_rec: exon_rec.start)
    ...
    def missing_chrom(self, num_processors_i):
        ...
        with open(ref_gap_info_bedfile_s,"w") as ref_gap_info_bedfile_f,\
            open(tgt_gap_info_bedfile_s, "w") as tgt_gap_info_bedfile_f:
            print(1)
    ...
    def main_missing_genes_analysis(self, num_processors_i):
        ...
        ref_gene_bed_file_s = os.path.join(basic_input_dir_s, "ref.genes.bed")
        ...
        if file_check(ref_gene_bed_file_s) is False:
            self.gene_2_bed(mode_s="ref")
        if file_check(ref_intergenic_bed_file_s) is False:
            self.intergenic_2_bed(mode_s="ref")
        if file_check(ref_exon_bed_file_s) is False:
            self.exon_2_bed(mode_s="ref")
        if file_check(ref_cds_bed_file_s) is False:
            self.cds_2_bed(mode_s="ref")
        #if file_check(ref_intron_bed_file_s) is False:
        ...
        with open(compare_missing_file_s,"w") as compare_missing_file_f:
            ...
            for position_s in ["genes", "intergenic", "exon", "cds"]: # "intron","cds"]:
                print("position: " + position_s)
    ...
    def gene_2_bed(self, mode_s):
        ...
        gene_bedfile_s = os.path.join(result_dir, "basic_inputs", "%s.genes.bed" % mode_s)
        gene_bedlines_l = []
    ...
    def intergenic_2_bed(self, mode_s):
        ...
        os.system("%s sort -i %s | %s merge -i stdin | %s subtract -a %s -b stdin > %s" %
                  (bedtools_command_s, gene_bedfile_s,
                   bedtools_command_s,
    ...
    def main_split_genes_analysis(self, fgl_s):
        ...
        with open(summary_of_blast_hit_s,"w") as summary_of_blast_hit_f:
            for gene_name_s in gene_2_split_d.keys():
                ...
                if gene_locus_id_s in split_ids_with_blast_hit_l:
    ...
    def mp_summary_mpileup_whole_genome(self, num_processors_i):
        ...
        for mode_s in [ref_s, tgt_s]:
            ...
            if file_check(mpileup_summary_file_s) is False:
                mpileup_file_s = "./%s.mpileup" % mode_s if mode_s == "vgp_calann" else "/disk3/juwan_d3/%s.mpileup" % mode_s
    ...
    def read_mapping_result_summary(self):
        ...
        with open(outfile_s, "w") as outfile_f:
            ...
            for fgl_s in ["frameshift", "prematurestopcodon", "intronexonjunctiondisruption"]:
                ...
                finch_summary_file_s = os.path.join(cat_rerun_dir_s, "zebra_finch", "tm", "CAT2MISSING_pre_taegut", fgl_s, "locus_wise.mpileup_summary.tabs")
                hummingbird_summary_file_s = os.path.join(cat_rerun_dir_s, "anna", "tm", "CAT2MISSING_pre_calann", fgl_s, "locus_wise.mpileup_summary.tabs")
                platypus_summary_file_s = os.path.join(cat_rerun_dir_s, "platypus", "tm", "CAT2MISSING_pre_ornana", fgl_s, "locus_wise.mpileup_summary.tabs")
                perch_summary_file_s = os.path.join(cat_rerun_dir_s, "climbing_perch", "tm", "CAT2MISSING_pre_anates", fgl_s, "locus_wise.mpileup_summary.tabs")
    ...
    def count_fgl_number(self):
        ...
        totally_missing_summary_file_s = os.path.join(result_dir, "exonDeletion", "totally_missing_and_exon_deletion.tabs")
    ...
    def add_UTR_manually(self, DB, mode_s):
        ...
        for gene_rec in gene_recs_l:
            ...
            gene_strand = gene_rec.strand
            gene_chrom_s = gene_rec.chrom
            # (3) if multiple transcripts for one gene --> handle each mRNA record
            for mRNA_rec in DB.children(id=gene_id_s, featuretype="mRNA"):
                mRNA_id_s = mRNA_rec.id
                mRNA_name_s = mRNA_rec["transcript_id"][0]
                gene_dbxref_id_s = gene_rec["Dbxref"][0] if mode_s in ("ref", "prev") else mRNA_rec["dbxref"][0]
                if mode_s == "ref":
                    gene_id_i = gene_dbxref_id_s.split(":")[1]
                elif mode_s == "tgt":
                    gene_id_i = gene_rec.attributes["source_gene"][0]
                else:
                    assert mode_s == "prev"
                    gene_id_i = gene_dbxref_id_s.split(" ")[0].split(":")[-1]
                ...
                for exon_rec in exon_recs_l:
                    ...
                    if (left_type1_bool | left_type2_bool) is True:
                        ...
                        utr_index_i += 1
                        info_line_s = ";".join(["ID=%s_%s_%d" % (left_utr_type_s, gene_id_i, utr_index_i),
                    ...
                    if (right_type1_bool | right_type2_bool) is True:
                        if right_type1_bool is True:
                            ...
                            right_utr_end_i = exon_end_i
                        utr_index_i += 1
                        info_line_s = ";".join(["ID=%s_%s_%d" % (right_utr_type_s, gene_id_i, utr_index_i),
                                                "Parent=%s" % (mRNA_id_s), "transcript_id=%s" % (mRNA_id_s),
                                                "Dbxref=%s,Genbank:%s" % (gene_dbxref_id_s, mRNA_id_s)])
    ...
    def get_representative(self, DB, input_gff_s, out_gff_s):
        """
    ...
    def gene_structure(self, mode_s):
        ...
        if mode_s == "ref":
            ...
            self.get_representative(DB=before_representative_db, input_gff_s=before_representative_gff_s, out_gff_s=representative_gff_s)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-eu", "--isEukaryotes", action="store", help="boolean, True if it is eukaryotic organism", default = False)
    ...
    eukaryote_bool = args.isEukaryotes
    ...
    tarGFF_s = os.path.join(cons_dir, "%s.gff3" % tgt_s)
    species_fasta_dir_s = os.path.join(fasta_dir_s)
    ...
    # customizing
    CDS_refGFF_s = refGFF_s.replace("converted.gff", "CDS.converted.gff")
    tRNA_refGFF_s = refGFF_s.replace("converted.gff", "tRNA.converted.gff")
    rRNA_refGFF_s = refGFF_s.replace("converted.gff", "rRNA.converted.gff")
    CDS_refDB = tmp_db_maker(gff_s=CDS_refGFF_s, name_s="CDS_ref", working_dir_s=result_dir,
                             featuretype_s="CDS")
    tRNA_refDB = tmp_db_maker(gff_s=tRNA_refGFF_s, name_s="tRNA_ref", working_dir_s=result_dir,
                              featuretype_s="tRNA")
    rRNA_refDB = tmp_db_maker(gff_s=rRNA_refGFF_s, name_s="rRNA_ref", working_dir_s=result_dir,
                              featuretype_s="rRNA")
    CDS_refDB.update(tRNA_refDB, merge_strategy="replace")
    CDS_refDB.update(rRNA_refDB, merge_strategy="replace")
    refDB = CDS_refDB
    ...
    ref_all_gene_recs_l, ref_all_trans_recs_l = gene_and_transcript_records(mode_s="ref")
    tgt_all_gene_recs_l, tgt_all_trans_recs_l = gene_and_transcript_records(mode_s="tgt")
    ...
    if work_s.lower() == "allinone":
        ...
        a.genome_gc_n_repeat_summary(window_size_i=1000)
